 ...
 *
 * @link       https://github.com/AgriLife/agrilife-learn-epubs/blob/master/src/class-taxonomy.php
 * @since      0.1.0
...
class Taxonomy {
	...
	public function __construct( $name, $slug, $post_slug = null, $user_args = array(), $meta = array(), $template = '' ) {
		...
		$labels = array(
			...
			'search_items'      => __( 'Search', 'agrilife-learn-epubs' ) . " $plural",
			'all_items'         => __( 'All', 'agrilife-learn-epubs' ) . " $plural",
			'parent_item'       => __( 'Parent', 'agrilife-learn-epubs' ) . " $singular",
			'parent_item_colon' => __( 'Parent', 'agrilife-learn-epubs' ) . " {$singular}:",
			'edit_item'         => __( 'Edit', 'agrilife-learn-epubs' ) . " $singular",
			'update_item'       => __( 'Update', 'agrilife-learn-epubs' ) . " $singular",
			'add_new_item'      => __( 'Add New', 'agrilife-learn-epubs' ) . " $singular",
			/* translators: placeholder is the singular taxonomy name */
			'new_item_name'     => sprintf( esc_html__( 'New %d Name', 'agrilife-learn-epubs' ), $singular ),
			'menu_name'         => $plural,
		);
